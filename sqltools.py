...
def get_length_column(url):
    global haveget
    arg1 = [(haveget['table']['curr'][i + 1],j,url) for i in range(haveget['table']['curr'][0]) for j in range(haveget['length'][haveget['table']['curr'][i + 1]]['c0'])]
    with mp.Pool(core_num) as p:
